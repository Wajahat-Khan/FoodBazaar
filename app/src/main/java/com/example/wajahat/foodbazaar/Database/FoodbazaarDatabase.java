    ...
    public static FoodbazaarDatabase getInstance(final Context context){
        if(Instance==null){
            synchronized (FoodbazaarDatabase.class){
                if(Instance==null){
                    Instance= Room.databaseBuilder(context.getApplicationContext(),FoodbazaarDatabase.class,"foodbazaar").fallbackToDestructiveMigration().addCallback(sRoomDatabaseCallback).build();
                }
            }
        }
        return Instance;
